...
if __name__ == '__main__':
    ...
    network_pkl = "networks/stylegan3-r-ffhqu-256x256.pkl"
    #truncation_psi = 1
    #noise_mode = "const"
    ...
    with dnnlib.util.open_url(network_pkl) as f:
        netD = legacy.load_network_pkl(f)['D'].to(device)
    ...
    for epoch in range(num_epochs):
        ...
        # output = netD(real_cpu, None).view(-1)
        # output = sig(output)
        # print(output)
        ...
        #if (iters % 500 == 0) or ((epoch == num_epochs-1) and (i == batch_size-1)):
        with torch.no_grad():
